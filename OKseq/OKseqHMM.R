...

{
  ...
  if (paired)
  {
    ...
    system(paste0("samtools view -@ ", threads, " -b -f 128 -F 16 ",bamfile," > a.fwd1.bam"))


    # include reads that are mapped to the reverse strand (16) and
    # first in a pair (64): 64 + 16 = 80
    system(paste0("samtools view -@ ", threads, " -b -f 80 ",bamfile," > a.fwd2.bam"))

    # combine the temporary files
    system(paste0("samtools merge -@ ", threads, " -f ",fileOut,"_fwd.bam a.fwd1.bam a.fwd2.bam"))
    ...
    system(paste0("samtools view -@ ", threads, " -b -f 144 ",bamfile," > a.rev1.bam"))

    # include reads that are first in a pair (64), but
    # exclude those ones that map to the reverse strand (16)
    system(paste0("samtools view -@ ", threads, " -b -f 64 -F 16 ",bamfile," > a.rev2.bam"))


    # merge the temporary files
    system(paste0("samtools merge -@ ", threads, " -f ",fileOut,"_rev.bam a.rev1.bam a.rev2.bam"))
  ...
  else
  {
    ...
    print("Seperating the forward strand bam.")
    # Forward strand.
    system(paste0("samtools view -@ ", threads, " -bh -f 16 ",bamfile," > ",fileOut,"_fwd.bam"))
    system(paste0("samtools index ",fileOut,"_fwd.bam"))

    print("Seperating the reverse strand bam.")
    # Reverse strand
    system(paste0("samtools view -@ ", threads, " -bh -F 16 ",bamfile," > ",fileOut,"_rev.bam"))
    system(paste0("samtools index ",fileOut,"_rev.bam"))
  ...
  for (i in c(1:length(chrom))){
    ...
    system(paste0("samtools view -@ ", threads, " ",fileOut,"_fwd.bam ",chr.name," > fwd_",chr.name,".sam"))
    ...
    system(paste0("samtools view -@ ", threads, " ", fileOut,"_rev.bam ",chr.name," > rev_",chr.name,".sam"))

...
OKseqOEM <- function(bamInF, bamInR, chrsizes, fileOut, binSize, binList)
{
  ...
  for (i in c(1:nrow(chr.sizes))){
    ...
    if (paired)
    {
      ...
      system(paste0("samtools view -@ ", threads, " -q 1 -f 0x42 -F 0x4 ",bamInF," ",chr.name," > fwd_",chr.name,".sam"))
      ...
      system(paste0("samtools view -@ ", threads, " -q 1 -f 0x42 -F 0x4 ",bamInR," ",chr.name," > rev_",chr.name,".sam"))
      ...
      system(paste0("samtools view -@ ", threads, " ",bamInF," ",chr.name," > fwd_",chr.name,".sam"))
      system(paste0("awk '$3~/^", chr.name, "$/ {print $2 \"\t\" $4}' fwd_",chr.name,".sam > fwd_",chr.name,".txt"))

      print(paste0("Calculating ",binSize,"bp binsize coverage for reverse strand."))
      system(paste0("samtools view -@ ", threads, " ",bamInR," ",chr.name," > rev_",chr.name,".sam"))
      system(paste0("awk '$3~/^", chr.name, "$/ {print $2 \"\t\" $4}' rev_",chr.name,".sam > rev_",chr.name,".txt"))
    }
